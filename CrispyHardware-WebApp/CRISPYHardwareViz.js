...
var refThingstreamServer = 'https://chrispyhardware-thingstreams.firebaseio.com' // URL of the firebase
...
/********************/
var voltmeterListVizInfo = { // TODO: get these *Info objects from cloud Viz name-server
		limit : 6,
		title : 'Recent Volts Measured',
		VizName : 'voltmeterList',
		VizTitle : 'voltmeterListTitle'
	}
...
function appendVoltmeterList(d3Selection) { // use with d3.call()
	appendList(d3Selection, voltmeterListVizInfo)
	d3Selection.select('[name=' + voltmeterListVizInfo.VizTitle + ']')
		.text(voltmeterListVizInfo.title)	
}
...
function redrawVoltmeterList(d3Data) { // called on data change
	redrawList.call(this, d3Data, function (d) {
			return '> ' + d.string + 'V was measured on ' + millisToLocalTimeStringFriendly(d.timeSessionStart)
		})
} //redrawVoltmeterList()

function onValueVoltmeter(snap) {
	var d3Data = convertFBSnapToD3Data(snap)
	redrawVoltmeterList.call(this, d3Data)
}
...
function addVizFromThingUuid(thingUuid, info) {
	...
	// delete context.recentVoltmeters // FIXME: Why did I need this?  I forgot.
} // addVizFromThingUuid(...
